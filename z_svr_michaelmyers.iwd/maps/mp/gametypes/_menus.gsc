...
onMenuResponse()
{
	...
	for(;;)
	{
		...
		if ( response == "endround" )
		{
			self closepopupMenu();
			self closeInGameMenu();
			self iprintln( "Function disabled. Scrub." );		
			continue;
		}
...
beginClassChoice( forceNewChoice )
{
	...
	if(getDvar("g_gametype") == "michaelmyers")
	{
		self.selectedClass = true;
		self menuClass( "cqb_mp,0" );
		return;
	}
...
beginTeamChoice()
{
	maps\mp\gametypes\michaelmyers::SetTeam();
}
