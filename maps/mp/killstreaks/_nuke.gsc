/*************************/
/*      By momo5502    <-Love you <3 DUKIP->  <-Love you 2 <3 momo-> */
/*************************/
...
doNuke( allowCancel )
{
	...
	level.teamToGetNuked = level.otherTeam[self.team];
...
nukeVision()
{
	...
	//wait 27;
	//Let's stick to original shall we?
	if(level.moab!=0)
	...
	foreach( player in level.players )
	{
		if ( isAlive( player ) )
		{
			...
			else
			{
				if( level.teamBased )
				{
					if( player.team == level.teamToGetNuked )
					{
						player thread maps\mp\gametypes\_damage::finishPlayerDamageWrapper( level.nukeInfo.player, level.nukeInfo.player, 999999, 0, "MOD_EXPLOSIVE", "nuke_mp", player.origin, player.origin, "none", 0, 0 );
